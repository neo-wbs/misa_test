...
def _append_event(db: Session, aggregate_id: str,
    ...
    db.add(event)
    # KEIN db.commit() hier — wird zusammen mit Projektion committed
...
# POST /links  — Command: schreibt in Event Store + aktualisiert Read-Model
@app.post("/links", status_code=201)
def save_link(body: SaveLinkRequest,
              auth: dict = Depends(get_current_user),
              db: Session = Depends(get_db)):
    start = time.time()
    # Nächste ID + Version bestimmen
    ...
    # 1. Event Store (Write-Modell) — append-only
    _append_event(db, f"link_{link_id}", "LinkGespeichert", payload, version)

    # 2. Projektion — Read-Modell aktualisieren (synchron)
    _project(db, "LinkGespeichert", payload)

    # 3. Beides in einer Transaktion committen (mini SAGA: alles oder nichts)
    db.commit()
    LINKS_COUNTER.inc()  # ← zählen
    LINK_DURATION.observe(time.time() - start)  # ← Zeit messen
    return {"id": link_id, "url": body.url, "title": body.title,
            "tags": body.tags}

# GET /links  — Query: liest nur aus Read-Model (kein Event Store!)
@app.get("/links")
...
# PATCH /links/{id}  — Command: Event Store + Projektion
...
def patch_link(link_id: int, body: PatchLinkRequest,
    ...
    # Version: letzte Version für dieses Aggregate + 1
    last = (db.query(Event)
...
# DELETE /links/{id}  — Command: Event Store + Projektion
...
# GET /links/{id}/history  — Bonus: Event Store direkt abfragen (Zeitreise!)
...
# Admin-Endpoint — alle Links aller User
@app.get("/admin/links")
def all_links(auth: dict = Depends(require_admin),   # ← nur admin!
              db: Session = Depends(get_db)):
